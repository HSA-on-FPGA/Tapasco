...
static int __init fflink_init(void)
{
	...
	err = char_hsa_register();
	if (err) {
		fflink_info("Could not register HSA char device\n");
		goto error_hsa_register;
	}

	err = char_hsa_dma_register();
	if (err) {
		fflink_info("Could not register HSA DMA char device\n");
		goto error_hsa_dma_register;
	}
